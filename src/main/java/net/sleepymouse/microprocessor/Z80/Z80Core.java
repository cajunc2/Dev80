...
import net.sleepymouse.microprocessor.CPUData;
 ...
 */
public class Z80Core implements CPUData
{
	/**
	 * All supported processor registers which can be accessed externally to the
	 * core
	 */
	...
	private final static byte[]    OPCODE_T_STATES          = new byte[] { 4, 16, 7, 6, 4, 4, 7, 4, 4, 11, 7, 6, 4, 4,
	        7, 4, // 0
	...
	private final static byte[]    OPCODE_CB_STATES         = new byte[] { 8, 8, 8, 8, 8, 8, 15, 8, 8, 8, 8, 8, 8, 8,
	        15, 8, // 0
	...
	private final static byte[]    OPCODE_DD_FD_STATES      = new byte[] { 0, 0, 0, 0, 0, 0, 0, 0, 0, 15, 0, 0, 0, 0,
	        0, 0, // 0
	...
	private final static byte[]    OPCODE_ED_STATES         = new byte[] { 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8,
	        8, // 0
	...
	private final static byte[]    OPCODE_INDEXED_CB_STATES = new byte[] { 0, 0, 0, 0, 0, 0, 23, 0, 0, 0, 0, 0, 0, 0,
	        23, 0, // 0
	...
	/**
	 * Standard constructor. Set the processor up with a memory and I/O
	 * interface.
	 * 
	 * @param ram
	 ...
	 */
	public Z80Core(Memory ram, IODevice io)
	{
		this.ram = ram;
		this.io = io;
		tStates = 0;
	...
	/**
	 * Indicate when a block move instruction is in progress, LDIR, CPDR etc.
	 * May be sampled during repetitive cycles of
	 * the instruction
	 * 
	...
	/**
	 * Initiate a maskable interrupt request
	 */
	public void setINT()
	{
		INT_FF = true;
	...
	/**
	 * Set the reset program address. This is the address the processor will
	 * fetch its first instruction from when
	 * reset.
	 * 
	 * @param address
	 *            Value in the range 0x0000 to 0xFFFF
	...
	/**
	 * Execute a single instruction at the present program counter (PC) then
	 * return. The internal state of the processor
	...
	public void executeOneInstruction() throws ProcessorException
	{
		...
		if(NMI_FF)
		{
			...
			else
			{
				...
				if(halt)
				{
					incPC(); // Was a bug ! - point to instruction after(!)
					         // interrupt location. HALT decrements PC !!!
				}
			...
			else
			{
				...
				switch(intMode) {
					case 0:
						...
						break;
					case 1:
						reg_PC = 0x0038; // Mode 1 Interrupt routine location
						break;
					case 2:
						int vec = reg_I << 8;
						vec += io.IORead(0) & 0b11111110;
						reg_PC = vec;
						break;
					default:
				}
	...
	/**
	 * Execute all one byte instructions and pass multi-byte instructions on for
	 * further processing
	 * 
	 * @param opcode
	 *            Instruction byte
	...
	private void extendedCB()
	{
		...
		switch (instruction)
		{
			...
			//
			// Undocumented SLL [0x30 to 0x37]. Instruction faulty, feeds in 1
			// to bit 0
			case 0x30:
			{
	...
	/* DAA is weird, can't find Zilog algorithm so using +0110 if Nibble>9
	 * algorithm. */
	private void DAA()
	{
		{
	...
	/**
	 * Note: This implemented the broken (and undocumented) SLL instructions.
	 * Faulty as it feeds in a zero into bit 0
	 * 
	 * @param temp
	 *            Register value
	 * @return
